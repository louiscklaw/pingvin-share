...
const EditableUpload = ({
  ...
  const setFiles = (files: FileListItem[]) => {
    const _uploadFiles = files.filter((file) => 'uploadingProgress' in file) as FileUpload[];
    const _existingFiles = files.filter((file) => !('uploadingProgress' in file)) as FileMetaData[];
  ...
  const save = async () => {
    ...
    try {
      ...
      const hasFailed = uploadingFiles.some((file) => file.uploadingProgress == -1);
  ...
  useEffect(() => {
    ...
    const fileErrorCount = uploadingFiles.filter((file) => file.uploadingProgress == -1).length;
