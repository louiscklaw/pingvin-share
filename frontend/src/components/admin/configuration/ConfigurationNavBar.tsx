import { Box, Button, createStyles, Group, MediaQuery, Navbar, Stack, Text, ThemeIcon } from '@mantine/core';
import Link from 'next/link';
import { Dispatch, SetStateAction } from 'react';
import { TbAt, TbMail, TbShare, TbSocial, TbBucket, TbBinaryTree, TbSettings, TbScale } from 'react-icons/tb';
...
const ConfigurationNavBar = ({
  ...
  return (
    <Navbar p="md" hiddenBreakpoint="sm" hidden={!isMobileNavBarOpened} width={{ sm: 200, lg: 300 }}>
      <Navbar.Section>
          ...
          {categories.map((category) => (
            ...
            >
              <Group>
                ...
                <Text size="sm">
                  <FormattedMessage id={`admin.config.category.${category.name.toLowerCase()}`} />
                </Text>
              </Group>
            </Box>
          ))}
